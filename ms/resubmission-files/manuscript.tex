...
RNA-seq experiments of \(>500\) human dorsolateral prefrontal cortex (DLPFC)
...
much higher estimates. Moreover, the handful of novel putatively imprinted
genes we do find are all in close genomic proximity to known imprinted genes.
...
In contrast, allelic bias appeared independent of schizophrenia.  
...
score, \(S_{ig}>0.7\), yielded similar ranking (Fig.~S11). The
...
clusters (Fig.~S1); 21 of these 29 are \emph{known imprinted}
...
but whose score was still substantial (Fig.~S2) yielding 30
...
explanatory variables (Table~S1,
Fig.~S3).  For both the fixed and mixed class we
...
and~S4).  This apparent dependence might be indirect, i.e.~one
...
effect (Fig.~S10 top middle).  The predicted coefficients
...
containing only 15 genes.  The results (Table~S3) are
...
(Table~S1) using several multiple regression models.  Based
on their structure our models can be classified into two sets of fixed and a
set of mixed regression models (Fig.~S5).  Furthermore
...
count ratio; see model classes in Table~S2).

Fig.~S5 explains that among the two fixed and the mixed
...
do provide (Fig.~S9).  Instead of being estimated,
...
Fig.~S10 for predicted gene-specific coefficients under a
mixed model).  Nonetheless, the low power and low robustness of fixed models
became apparent from results like those in Fig.~S9 so
...
fit of the non-structural types (Fig.~S8) and found
...
(Table~S2).  For inference we used the best fitting
combination (\(\mathrm{unlm.Q}\), Table~S2) as assessed by
the normality and homoscedasticity of residuals (Fig.~S8,
also Fig.~S6,~S7) as well
...
\newblock {Gene expression elucidates functional impact of polygenic risk for
  schizophrenia.}
\newblock {\em Nature neuroscience}, sep 2016.
