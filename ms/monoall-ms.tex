...
characteristic to imprinting.  Consistently 29 of the top-scoring 50
genes fell into previously described imprinted gene clusters
...
\(S_{\cdot g}\) distribution is due only to mapping bias for \emph{distant
candidates} but predominantly to imprinting for \emph{known imprinted} and
\emph{nearby candidate} genes.  This was further supported by the finding that
only for \emph{distant candidate} genes do many individuals display read count
ratio consistent with nearly unbiased allelic expression (black bars in
Fig.~\ref{fig:top-genes}, Methods), which suggests that without the likely
mapping bias in the remaining individuals the distribution of \(S_{\cdot g}\)
in these genes would be shifted to the left indicating no imprinting.

Based on these results we called all \emph{known imprinted} and \emph{nearby
candidate} genes within the top 50 imprinted in the adult human DLPFC and
included also the \emph{known imprinted} gene UBE3A, which ranked below 50 but
whose score was still substantial (Fig.~\ref{fig:known-genes}).  These 30
imprinted genes are listed in the panels of Fig.~\ref{fig:S-Dx}, which shows
that read count ratio is similarly distributed in the Control, SCZ and AFF
group suggesting independence between allelic bias and diagnosis of
schizophrenia.
...
independent of age for the rest of imprinted genes.  Yhis apparent
...
intriguing.  Age dependence during embryonic development and
childhood is both well-supported experimentally and well-understood but that
