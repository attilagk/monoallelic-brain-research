...
\(i\) (dots) and 30 genes \(g\) that are imprinted in the DLPFC brain area.
}
...
\end{center}
\caption{ 
%Estimate and $99\%$ confidence interval for each
%regression coefficient \(\beta_{jg}\) under the wnlm.Q model.  The sign of an
%estimate \(\hat{\beta}_{jg}\) reports on the direction The mechanistic
%interpretation of depends on the nature of
%the predictor associated with \(j\); for example if the predictor is causal to parental
%bias then \(\hat{\beta}_{jg}\) is the effect size of the predictor.
}
...
We have presented the first genome-wide characterization of parental
...
The above conclusions depend on the correctness of our statistical
...
limitations (Section~\ref{sec:limitations}).  To quantify their impact future
simulation studies will be needed.  Adopting a hierarchical Bayesian framework
(Fig.~\ref{fig:glm-vs-hierarch} right) would help overcome the present
limitations via information sharing across genes, via optimal treatment of
uncertainty about genotyping and about haplotype, and via drawing
information from more predictors and accounting for their interactions.  Such
improvements would be critical for addressing the question of other
psychiatric disorders such as affective spectrum, of other brain regions, and
of DNA methylation patterns.  Another challenging aim is direct comparability
between human genomic studies and designed mouse experiments, which would
enable the discovery of putative human-specific signatures of imprinting.
That, in turn, would help elucidate the (epi)genetics of normal and
dysfunctional traits of human psychology and cognition.
