...
An in-depth analysis of model fit was carried out involving all models,
...
checking results suggest that for all genes the normal linear model fits at
...
(wnlm.S).  On the other hand, both logistic models exhibited greatly variable
fit across genes but the fit of logi.S was in general better than that of
...
gene \(g\) and parameter under the normal linear (wnlm.Q) and logistic
(logi.S) model revealed good qualitative agreement unless logi.S failed to fit
the data well for that gene.  The comparison also revealed that logi.S is more
powerful in detecting a significant effect and rejecting the null hypothesis
\(\beta_{jg}=0\), which is in line with the mentioned theoretical advantages
of logistic models.  The p-value for each null hypothesis was calculated not
only the standard way based on normal distribution theory but also from a
random permutation test (Fig.~\ref{fig:p-val-tdist-vs-perm}).  The two
approaches to the p-value agreed closely under wnlm.Q for all genes while
