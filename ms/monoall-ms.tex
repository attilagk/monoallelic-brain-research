...
We modeled the dependence of read count ratio of imprinted genes on biological and technical explanatory variables (Table~\ref{tab:predictors})
using two generalized linear regression frameworks
\begin{enumerate}
\item hierarchical mixed models, which describe data \emph{jointly} for all 30 imprinted genes
\item non-hierarchical fixed models, which describe data
\emph{separately} for each of the 30 imprinted genes TODO: equation
\end{enumerate}

Mixed models can be written as \(g^{-1}(y) &=& X \beta + Z b +
\varepsilon\) and fixed models as \(g^{-1}(y) &=& X \beta + 
\varepsilon\).  In this formalism \(g\) is a link function; the response \(y\) is the
observed read count ratio that is possibly transformed to improve fit; \(X, Z\) are design matrices
containing data on explanatory variables; \(\beta \) and \(b\) are
fixed and random vectors of regression coefficients, respectively;
\(\varepsilon\) is the random error.  Besides the presence or absence of
random effects term \(Zb\) the key difference between
the mixed and fixed models in this study is
...
We formulate the model so that the response variable \(y\)---the read count ratio that may
have been transformed to improve fit---is 
\begin{eqnarray}
\label{eq:nlme-matrix}
y &=& X \beta + Z b + \varepsilon \\
b &\sim& \mathcal{N}(0, \Omega_b) \\
\varepsilon &\sim& \mathcal{N}(0, \sigma^2 I_{m n_1})
\end{eqnarray}

Eq.~\ref{eq:nlme-matrix}

\begin{eqnarray}
y_i = \overbrace{\sum_{j=0}^J x_{ij} \beta_j}^{\text{fixed effects}} +
\overbrace{\sum_{k=1}^K \sum_{j=0}^{J_k} z_{ij}^{(k)} b_{j}^{(k)}}^{\text{random effects}} + \varepsilon_i
\end{eqnarray}
