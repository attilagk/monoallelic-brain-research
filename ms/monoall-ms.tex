...
on \(m=579\) individuals and \(n=30\) imprinted genes and so the response vector is
...
ratio is dependent on diagnosis as well as other explanatory variables. These
included biological variables namely Age, the first five principal components
of ancestry (Ancestry.1,...,Ancestry.5), Gender as well as technical variables
such as RIN, the RNA integrity number (see Table~\ref{tab:predictors} for
complete list).  Our mixed models regress read count ratio on some or all of
the explanatory variables simultaneously, which allows the dissection of
various biological and technical sources of variation.  Another benefit of the
mixed modeling framework is its power because it makes use of information that
is only available when all thirty genes are considered simultaneously, while
also allowing genes to be considered individually.  These benefits however
...
Based on the selected model we formally tested whether read count ratio
...
for \(g\)) and the corresponding explanatory variables \(X_1,...,X_3\).  For simplicity
but without loss of generality only 3 explanatory variables are depicted.  The model
frameworks differ in how \(\beta_{jg_1},\beta_{jg_2},...\) relate to each
other for a given explanatory variable (or a given \(j\)).  \emph{Left:} there is no
connection among \(\beta_{jg_1},\beta_{jg_2},...\) which means that the way
\(Y_{g}\), the read count ratio for gene \(g\) depends on variable \(X_j\) is
...
with respect to how their read count ratio depends the explanatory variables.  Thus genes
