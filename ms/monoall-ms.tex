...
interpreted as effects on allelic bias.
Predictor terms whose effect on \(Q\) is modeled as random are enclosed in
parentheses.  Terms like \((1\,|\,\mathrm{Dx})\) or \(\mathrm{Age}\) denote effects that are
uniform across imprinted genes (gene non-specific), while terms like
\((1\,|\,\mathrm{Dx}:\mathrm{Gene})\) or \((\mathrm{Age}\,|\,\mathrm{Gene})\) mean that the effect is specific to
certain imprinted gene(s). Strongly negative \(\Delta\mathrm{AIC}\) and small
p-value indicate significant dependence (see Methods).
}
\label{tab:mod-sel}
\end{table}
