...
579 human dorsolateral prefrontal cortex (DLPFC) samples.  We find that the
...
the diagnosis of schizophrenia is independent of allelic bias.  In contrast
...
imprinted genes in the human brain and (ii) explaining the
...
(Fig.~\ref{fig:study-design}), which ranges from 0.5 to 1 indicating unbiased
biallelic expression (at 0.5), some allelic bias (at intermediate values) or
strictly monoallelic expression (at 1) (Methods).  We corrected for a number
...
distributions suggest large across-individuals variation of allelic bias for all
genes, even if a substantial component of the variation originates from technical sources.
...
characteristic to imprinting.  Consistent with this, 29 of the top-scoring 50
genes fell into previously described as imprinted gene clusters
(Fig.~\ref{fig:clusters}).

Fig.~\ref{fig:top-genes} shows that 21 of the 29 within-cluster genes are
\emph{known imprinted} and 8 are not (termed \emph{nearby candidates} for
imprinting); the 21 genes falling outside clusters are termed \emph{distant
candidates}.  For 37 of the top 50 genes data was sufficient to test for
mapping bias (Methods).  The test results (see symbols to the right of gene
names in Fig.~\ref{fig:top-genes}) suggested that the strongly right-shifted
read count ratio distribution is exclusively mapping bias for \emph{distant
candidates} but predominantly imprinting for \emph{known imprinted} and \emph{nearby
candidate} genes.  This was further supported by the finding that only for
\emph{distant candidate} genes do many individuals display read count ratio
consistent with nearly unbiased allelic expression (black bars in
Fig.~\ref{fig:top-genes}, Methods), which suggests that without the likely mapping bias
in the remaining individuals the distribution of \(S_{\cdot g}\) would be
shifted to the left indicating no imprinting.

Based on these results we call all \emph{known imprinted} and \emph{nearby
candidate} genes within the top 50 imprinted in the adult human DLPFC and
include also the \emph{known imprinted} gene UBE3A, which ranked below 50 but
whose score was still substantial.  These 30 imprinted genes are listed in the
panels of Fig.~\ref{fig:S-Dx}, which shows that read count ratio is similarly
distributed in the Control, SCZ and AFF groups suggesting independence between
allelic bias and diagnosis of schizophrenia.
...
Fig.~\ref{fig:predictor-associations}), and which is made powerful by ``genes
borrowing strength from each other'' (Fig.~\ref{fig:glm-vs-hierarch}).  We
fitted many alternative mixed models, selected the best-fitting one using the
Akaike Information Criterion (AIC) and confirmed the goodness of fit with
diagnostic plots (Methods, Fig.~\ref{fig:qqnorm-mixed},~\ref{fig:homoscedas-mixed}).

Based on the best fitting model (henceforth ``the model'') we could formally
reject the hypotheses that read count ratio depends on diagnosis either as
main effect or interaction (see term \((1\mid\mathrm{Dx})\) and
\((1\,|\,\mathrm{Dx}:\mathrm{Gene})\) in Table~\ref{tab:mod-sel},
respectively).  That this key result is not due to low power is indicated
by the highly significant dependence of read count ratio on the gene in
question (see \((1\mid\mathrm{Gene})\) in Table~\ref{tab:mod-sel}).
...
independent of age for the rest of imprinted genes but this observed
dependence might be indirect, i.e.~one that is mediated by some variable(s)
``inbetween'' age and read count ratio
(Fig.~\ref{fig:glm-vs-hierarch},~\ref{fig:predictor-associations}).
...
no fixed effect---that would be shared by all imprinted genes---was supported
(see \((\mathrm{Age}\,|\,\mathrm{Gene})\) and \(\mathrm{Age}\), respectively,
in Table~\ref{tab:mod-sel}).  Based on the model we also predicted
...
which (e.g.~UBE3A) represent purely indirect dependence.
...
thousand, suggesting that the number of imprinted genesi in the brain is an
...
We have performed the most in-depth analysis up till now on the how 
imprinting depends on schizophrenia, as well as on age and other variables.  This was made possible by
(i)
more study individuals than in previous work~\cite{Baran2015} distributed in
well-balanced case-control study groups, and
(ii)
powerful statistical inference based on a mixed model.  Despite these advantages, technical variation is still large and
consequently so is the uncertainty of our statistical inferences.

Although our approach gave strong support for dependence of imprinting on
age and ancestry, no dependence on schizophrenia was detected either when
we assumed that the dependence is the same for all imprinted genes or that it
varies across genes.  This might seem to suggest that imprinting in the DLFPC
plays no significant role in schizophrenia and psychotic disorders
contradicting the ``imprinted brain'' hypothesis~\cite{Crespi2008}.
Alternatively,
imprinting does play a role in
schizophrenia but only very strong perturbations of some
imprinted genes increase the risk significantly, perturbations that are too
rare to detect in even at our relatively large sample size.  Additionally, the
more subtle perturbations in our data might still have a significant effect when
considered together with other genetic, epigenetic or environmental risk
factors that were absent in our model.  The complex genetic architecture of
schizophrenia~\cite{Sullivan2012} makes these alternative explanations 
quite plausible.
...
Our discovery that imprinting depends on age in later adulthood is rather
intriguing.  Although age dependence during embryonic development and
childhood is both well-supported experimentally and well-understood, that
...
statistically underpowered to address this question in humans and the only
experimental hints were gained from young mice~\cite{Perez2015}.  Although our
...
loss of tight regulation of those genes after because they loose their
functional significance in aging.
...
\subsection{Test and post hoc correction for mapping bias}

We performed a test to see if there is mapping bias for the top
ranking genes (Fig.~\ref{fig:top-genes}). For any gene (known imprinted, or
candidate) the expectation is that when some allelic bias is detected, that
...
the reference allele. The basis of the test is that for genes unaffected by
mapping bias the distribution of the number of individuals with reference
allele is expected to be binomial with probability parameter \(0.5\).  The
results of this and the following test guided our post hoc correction that
aims to remove certain genes despite their high score reflecting right-shifted
distribution of read count ratio.

\subsection{Testing nearly unbiased allelic expression}
...
gene indicates substantial variation of allelic bias.  The vertically arranged
...
\(a,b,c\) in green circles, respectively.  As expected, PEG10 and ZNF331 both score
...
indicate the fraction of individuals passing the test of nearly unbiased
expression (Eq.~\ref{eq:unbiased-test}).  ``X'' characters next to gene names
indicate mapping bias based on counts of reference and non-reference alleles, while ``0''
indicate that total allele count was insufficient for this test.
}
\label{fig:top-genes}
\end{center}
\end{figure}
...
\end{tabular}
\end{center}
\caption{Dependence of read count ratio on various predictor terms: largely
