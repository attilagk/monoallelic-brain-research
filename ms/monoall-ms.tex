...
genes while 8 are \emph{nearby candidates} defined as genes near \emph{known
imprinted} ones but themselves previously not
shown to be imprinted (blue and green \(y\)-axis labels in
Fig.~\ref{fig:top-genes}).   \emph{A priori} the expectation is that
\emph{known imprinted} genes and \emph{nearby candidates} are much more likely to
be imprinted  in the present data set than \emph{distant candidates} defined
as genes that neither belong to nor localize near (\(<1\mathrm{Mb}\)) \emph{known
imprinted} genes (Fig.~\ref{fig:top-genes}, red \(y\)-axis labels).  We combined this
...
To show this quantitatively we performed statistical inference using mixed
...
Fig.~\ref{fig:S-age} and~\ref{fig:Q-age} suggest that the read count ratio depends negatively on
...
fit to the data.  We performed fitting with the lme4 and stats R packages and
tested several combinations of transformations, link functions, and error
distributions (Table~\ref{tab:model-names}).  For inference we used the best
fitting combination (\(\mathrm{unlm.Q}\), Table~\ref{tab:model-names}) as
assessed by the normality and homoscedasticity of residuals
(Fig.~\ref{fig:qqnorm-mixed}) as well as by monitoring convergence.
...
(Fig.~\ref{fig:qqnorm-mixed}).  We have data
...
a \emph{mixed}
(lower middle) effects multiple regression model.  In all three model
structures the read count
ratio \(Y_g\)---for
several genes \(g\)---depends somehow on three explanatory variables \(X_j\).
For each gene \(g\) the probabilistic dependence is mediated by fixed \(\beta_{1g},\beta_{2g},\beta_{3g}\) or random
\(b_{2g},b_{3g}\) regression coefficients.  \emph{fixed II} is a
constrained version of the \emph{fixed I} model structure such that
\(\beta_{jg_1}=\beta_{jg_2}=...\equiv \beta_j\), which means that the effect
of \(X_j\) on \(Y\) does not vary across genes in \emph{fixed II}.
The \emph{mixed} model differs from \emph{fixed I}  
in the way coefficients across genes vary for a given explanatory variable
\(X_j\).  In the \emph{fixed I} model structure there
...
among the gene-specific substructures of \emph{fixed I}.  This limitation is
overcome with the \emph{mixed} model structure
because a set of coefficients across genes---e.g.~the set \(\{b_{2g}\}_g\))---is
modeled as a random sample from a normal distribution with 
parameters \(\mu_2\) and some \(\sigma^2_2>0\).  Thus \(\mu_2\) and
\(\sigma^2_2\) constitute information on the effect that is shared across
all genes.  Consequently, information on that effect is shared across all
genes and genes ``borrow strength from each other''.  When \(\sigma^2_j=0\) all parameters \(\{b_{jg}\}_g\) for \(X_j\) are
fixed at \(\mu_j\equiv\beta_j\), which is characteristic to the \emph{fixed
II} model structure.  In the \emph{mixed} model structure this is seen for \(X_1\), which therefore has the same
effect on \(Y_g\) for every gene \(g\).
In this example all explanatory variables are continuous in both models.  Any categorical
explanatory variable (factor) \(X_j\) with \(K\) levels would lead to \(K -
1\) fixed or random coefficients \(\beta_{j_1g},...,\beta_{j_{K-1}g}\) or 
\(b_{j_1g},...,b_{j_{K-1}g}\) for any gene \(g\), respectively.  Moreover, if the effect of that
categorical \(X_j\) is random then it is possible to have a continuous
\(X_{l'}\) with a random slope with respect to \(X_j\).  In fact the
\emph{mixed} model structure (lower middle) is equivalent to another one (not shown), where
``Gene'' is a random factor \(X_\mathrm{Gene}\) with random slope for the
effects of \(X_2\) and \(X_3\).
}
...
\end{center}
\caption{
Fitting four different fixed regression models (named logi.S, logi2.S, wnlm.S,
wnlm.Q) on the read count ratio data for the PEG3 gene.  For explanation of
model names see Table~\ref{tab:model-names}.
\emph{Upper left:}
...
\end{center}
\caption{
Fitting four fixed regression models (named unlm.S, unlm.Q, wnlm.S, wnlm.Q,
and logi.S) on read count ratio data for the KCNK9 gene.  The plots suggest
that unlm.Q and wnlm.Q fit the data the best.  See the legend of
Fig.~\ref{fig:fitting-fixed-PEG3} for further details. Convergence diagnostics
(not shown) suggested unlm.Q as the favored model for statistical inference.
}
...
\end{center}
\caption{
Predicted fixed coefficients \(\beta_{jg}\) for gene \(g\) (\(y\)-axis) and
fixed effect \(j\) (panel headers). Positive and negative coefficient indicates direct positive and
...
(Fig.~\ref{fig:qqnorm-mixed}) and monitoring
