...
Outline:
\begin{itemize}
\item we modeled the dependence of read count ratio of imprinted genes on
on biological and technical explanatory variables (Table~\ref{tab:predictors})
using two regression frameworks
\begin{enumerate}
\item hierarchical mixed effects models, which describe data jointly for all 30 imprinted genes
 TODO: equation
\item non-hierarchical generalized linear models, which describe data
separately for each of the 30 imprinted genes TODO: equation
\end{enumerate}
\item within each framework several data transformations, link functions, error distributions (Table~\ref{tab:model-names})
\item initial inference using non-hierarchical models guided heuristic search
for the best fitting mixed model considering the space of linear predictors
based on subsets of the explanatory variables
\item we prefered mixed models for final inference due to their higher power and
robustness against overfitting which they achieve by letting gene-wise data borrow statistical strength from each other
\end{itemize}
...
\begin{itemize}
\item overall hypothesis: aberrations in imprinted genes as a risk of
SCZ
\item specific hypothesis: normal allelic bias is perturbed in SCZ
\item Fig.~\ref{fig:S-Dx} compares read count ratio distribution among
control, SCZ and AFF and shows no difference
\item more rigorous test: mixed (hierarchical) regression model-based statistical inference considering data
for all 30 genes jointly
\item read count ratio as response variable whereas Dx, Gene, Age, RIN,... as
explanatory variables
\item identify the best fitting model (Methods) and quantify how significantly the presence
of various terms of explanatory variables in the model improves the fit
...
the gene-to-gene variation of read count ratio among imprinted genes (Fig.~\ref{fig:S-Dx})
\item two terms involving Dx: two sub-hypotheses
\begin{enumerate}
\item \((1\,|\,\mathrm{Dx})\) represents effect of Dx on read count ratio that
is independent of any other explanatory variable in the model and thus is not specific to any gene
...
to our mixed models once again, evaluating how significantly terms associated
with these variables improve fit (Table~\ref{tab:mod-sel})
\item as for Dx we defined two predictor terms for each variable, one that
...
significant 
\item TODO: predicted effect of Age that is specific to each gene
\footnote{similar prediction for Ancestry.1 is possible but seems much less
...
\hline
\((1\,|\,\mathrm{Gene})\)                   & Gene, independently of other variables & \(-126.8\)  & \(8.5\times 10^{-28}\) \\
\((1\,|\,\mathrm{Dx})\)                   & Dx, independently of other variables & \(2.0\)  & \(1.0\)  \\
\((1\,|\,\mathrm{Dx}:\mathrm{Gene})\)       & Dx, depending on Gene & \(0.4\) &  \(0.21\) \\
\(\mathrm{Age}\)                            & Age, independently of other variables & \(1.3\)   & \(0.39\) \\
\((\mathrm{Age}\,|\,\mathrm{Gene})\)        & Age, depending on Gene & \(-18.9\)  & \(2.5\times 10^{-5}\)  \\
\(\mathrm{Ancestry.1}\)                     & Ancestry.1, independently of other variables & \(0.6\)  & \(0.24\) \\
\((\mathrm{Ancestry.1}\,|\,\mathrm{Gene})\) & Ancestry.1, depending on Gene & \(-71.2\)   & \(4.6\times 10^{-16}\) \\
\((1\,|\,\mathrm{Gender})\)               & Gender, independently of other variables & \(2.0\)  & \(1.0\)  \\
\((1\,|\,\mathrm{Gender}:\mathrm{Gene})\)   & Gender, depending on Gene & \(-5.7\) &  \(5.5\times 10^{-3}\) \\
\end{tabular}
\end{center}
