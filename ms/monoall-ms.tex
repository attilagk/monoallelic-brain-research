...
To show this quantitatively we fitted several fixed and mixed effects models
...
set of mixed regression models (Fig.~\ref{fig:glm-vs-hierarch}).  Based on
non-structural properties (link function, error distribution, weighting, and
the data transformation to read count ratio) our models can be also classified
into the families summarized in Table~\ref{tab:model-names}).

Among the two fixed and the mixed structural model class the mixed one is both
more powerful and robust because its random effects terms allow gene-specific
parts of the model to ``borrow strength from each other'' as explained by
Fig.~\ref{fig:glm-vs-hierarch}.  The cost of the enhanced power in mixed
models is that for the random effects we do not get estimates and confidence
intervals for gene-specific coefficients (parameters) like for the fixed
effects coefficients (Fig.~\ref{fig:pred-fixed-coefs}) in fixed models.
Instead of being estimated, gene-specific coefficients can only be
\emph{predicted}, which means they lack of confidence intervals and p-values
of statistical significance  (see Fig.~\ref{fig:pred-rnd-coefs} for predicted
gene-specific coefficients under a mixed model).  Nonetheless, the low power
and low robustness of fixed models became apparent from results like those in
Fig.~\ref{fig:pred-fixed-coefs} so we based our final inference
(Table~\ref{tab:mod-sel}) on mixed modeling.  Note, however, that we found
overall qualitative agreement between mixed and fixed models regarding
gene-specific coefficients (compare Fig~\ref{fig:pred-rnd-coefs} and
\ref{fig:pred-fixed-coefs}).
