...
p)^{T-x}.\] The fact that this probability increases with \(p\),
the parental bias supports our intuition.
...
expression is biased, tends to 1 as \(p_{ig} \rightarrow 1\).  We used this
theoretical result in that we subjected only those genes to statistical
inference, whose read count ratio was found to be high and, therefore, whose
\(p_{ig}\) is expected to be high as well.
...
(black and gray silhouettes), age, or gender.  In addition to these, the
differences in the observed read count ratio also reflect variation from
technical sources like tissue preparation, or RNA sequencing.
}
...
\(x_{\cdot 1},...,x_{\cdot p}\) on features of study individuals that are not
gene-specific. We call these features \emph{predictors} because we used them
in a regression model framework.  Predictors and their
...
\(n=5307\) genes passed our filters designed to remove genes with scarce
RNA-seq data reflecting low expression and/or technical problems pertaining to
RNA sequencing.  On these we performed exploratory statistical analysis based
...
of \(S_{\cdot g}\) given each gene \(g\) he location and the scale of
\(S_{\cdot g}\).  The three plots of the upper half show the distribution for
only three genes but in three distinct representations, while the main panels of the
lower half present the distribution for all 5307 genes in the most compact
representation.  Two of the three genes in the upper half, PEG10 and ZNF331,
are \emph{known imprinted genes} in the sense that they had previously been
found imprinted in various tissue types, developmental stages and species.  The third,
AFAP1, is a \emph{candidate gene} as it lacks such evidence.  For
all three genes \(S_{\cdot g}\) varies considerably within its theoretical range
...
Fig.~\ref{fig:top-genes}) in the top 50 set are not
imprinted.  This was supported by two additional lines of evidence.  First, we
looked an aspect of the empirical distribution of \(S_{\cdot g}\)
that is more complex than the gene score.  This meant
testing the hypothesis for each gene \(g\) and individual \(i\) that allelic
expression is (nearly) unbiased
(Eq.~\ref{eq:unbiased-test}).  The fraction of individuals for which the test
was \emph{not} rejected tended to be much higher for the ``red'' genes in the
top 50 (black bars in Fig.~\ref{fig:top-genes}) consistent with our
classification.  The second line of evidence came from a test of false call
for heterozygous genotype (TODO: details).
...
(Eq.~\ref{eq:nlm-general}, \ref{eq:glm-mean-predictor}).
Of these terms Age and
Ancestry.1 represent either a continuous predictor whereas GenderMale and
DxSCZ are “treatment” levels of a categorical predictor (Gender and Dx,
respectively) contrasted with the corresponding control levels (GenderFemale
and DxControl, see Table~\ref{tab:predictors}).  We left out DxAFF because of
the small number of AFF individuals appeared to substantially decrease the power of the hypothesis
test described below.

For each term \(j\), and each gene \(g\) called imprinted, we tested the null
hypothesis \(\mathcal{H}_{jg} : \beta_{jg} = 0\) meaning that the read count
ratio is independent of that term (Fig~\ref{fig:pval-wnlm.Q}).  The
phenomenological interpretation of the rejection of \(\mathcal{H}_{jg}\) is
simply that parental bias probabilistically depends on the predictor.  The
