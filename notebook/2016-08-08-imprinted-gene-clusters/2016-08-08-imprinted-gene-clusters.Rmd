...
```{r cluster-sizes, echo=FALSE, fig.width=7, fig.asp=1}
gs.seg$segment <- factor(gs.seg$seg, levels = c(1:36, 0, - 1 * 1:36))
barchart(table(gs.seg$segment)[1:36], ylim = c(37, 0), xlab = "# genes in cluster (including <1M candidates)", ylab = "imprinted gene cluster")
```

```{r echo=FALSE}
beta.age$start <- gs[rownames(beta.age), "start"]
beta.age$chr <- gs[rownames(beta.age), "chr"]
beta.age$seg <- gs[rownames(beta.age), "seg"]
beta.age <- beta.age[with(beta.age, order(chr, start)), ]
beta.age$cluster <- as.factor(beta.age$seg)
levels(beta.age$cluster) <- paste("clus", seq_along(levels(beta.age$cluster)))
beta.age$Gene <- factor(beta.age$Gene, levels = beta.age$Gene, ordered = TRUE)
x <- with(beta.age, paste0(cluster, " (", chr, ")"), levels = unique(x), ordered = TRUE)
beta.age$clus.chr <- factor(x, levels = unique(x), ordered = TRUE)
```
...
(sp <- segplot(Gene ~ Lower.CL + Upper.CL | clus.chr, data = beta.age,
               ...
               par.settings = list(layout.heights = list(panel = sapply(levels(beta.age$cluster), function(k) sum(beta.age$cluster == k)))),
               xlim = c(-0.04, 0.04),
