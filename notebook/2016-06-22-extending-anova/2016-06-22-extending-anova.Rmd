...
```{r lattice-functions, cache=TRUE, warning=FALSE, echo=FALSE}
...
my.dotplot <- function(fm = Predictor ~ Deviance | Gene, data = A.long$logi.S, ...) {
    dotplot(fm,
              groups = Order,
              data = data[ data$Order %in% c("forward", "reverse"), ],
              pch = 21, abline = list(v = 0, lty = 2),
              #scales = list(x = "free"),
              auto.key = TRUE,
              aspect = 3, between = list(x = 0.2),
              ...)
}
```

Under `logi.S`:
```{r anova-fw-rv-logi.S, dev=c("png","pdf"), echo=FALSE, cache=FALSE}
my.stripplot(fm = Predictor ~ Deviance | Order, data = A.long$logi.S, main = "ANOVA under logi.S")
```

The same tendencies emerge under `wnlm.R`:
```{r anova-fw-rv-wnlm.R, dev=c("png","pdf"), echo=FALSE}
my.stripplot(fm = Predictor ~ Deviance | Order, data = A.long$wnlm.R, main = "ANOVA under wnlm.R")
```
...
Under `logi.S`:
```{r effects-fw-rv-logi.S, dev=c("png","pdf"), echo=FALSE}
my.stripplot(fm = Coefficient ~ Effect | Order, data = Ef.long$logi.S, main = "Effects under logi.S")
```

Again, similar tendencies are observed under `wnlm.R`:
```{r effects-fw-rv-wnlm.R, dev=c("png","pdf"), echo=FALSE}
my.stripplot(fm = Coefficient ~ Effect | Order, data = Ef.long$wnlm.R, main = "Effects under wnlm.R")
```

### Comparison with genes on relative scale

```{r anova-fw-rv-logi.S-trellis, dev=c("png","pdf"), echo=FALSE}
my.dotplot(fm = Predictor ~ Deviance | Gene, data = A.long$logi.S, main = "ANOVA under logi.S", scales = list(x = "free"))
```

```{r effects-fw-rv-logi.S-trellis, dev=c("png","pdf"), echo=FALSE}
my.dotplot(fm = Coefficient ~ Effect | Gene, data = Ef.long$logi.S, main = "Effects under logi.S", scales = list(x = "free"))
```

```{r anova-fw-rv-wnlm.R-trellis, dev=c("png","pdf"), echo=FALSE}
my.dotplot(fm = Predictor ~ Deviance | Gene, data = A.long$wnlm.R, main = "ANOVA under wnlm.R", scales = list(x = "free"))
```

```{r effects-fw-rv-wnlm.R-trellis, dev=c("png","pdf"), echo=FALSE}
...
my.segplot <- function(data = Betas$logi.S,
    ...
    segplot(Gene ~ Lower.CL + Upper.CL | Coefficient,
            data = data,
            #data = data[ ! data$Coefficient %in% "(Intercept)", ],
            ...
            main = main, as.table = TRUE, center = data$Estimate, draw.bands = FALSE, ...)
}
```

```{r effects-fw-rv-logi.S-trellis-coef-cond, dev=c("png","pdf"), fig.asp=1, echo=FALSE}
...
```{r reg-coef-logi.S, dev=c("png", "pdf"), fig.width=10, fig.asp=1, warning=FALSE, echo=FALSE, message=FALSE}
my.segplot(data = Betas$logi.S)[-1]
```

#### Under wnlm.R

```{r effects-fw-rv-wnlm.R-trellis-coef-cond, dev=c("png","pdf"), fig.asp=1, echo=FALSE}
# conditioning on the Coefficient instead of Gene
my.dotplot(fm = Gene ~ Effect | Coefficient, data = Ef.long$wnlm.R, main = "Effects under wnlm.R")
```

```{r reg-coef-wnlm.R, dev=c("png", "pdf"), fig.width=10, fig.asp=1, warning=FALSE, echo=FALSE, message=FALSE}
my.segplot(data = Betas$wnlm.R, main = expression(paste("99 % CI for ", beta, " under wnlm.R")))[-1]
```
