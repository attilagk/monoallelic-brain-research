...
(plots$effects.wnlm.Q <- my.stripplot(fm = Coefficient ~ Effect | Order, data = Ef.long$wnlm.Q, main = "Effects under wnlm.Q", aspect = length(levels(Ef.long$wnlm.Q$Coefficient)) / 10))
```
...
print(update(plots$anova.logi.S, main = "ANOVA", layout = c(2, 1), aspect = "fill", par.settings = list(superpose.symbol = list(cex = 0.5))), split = c(1, 1, 2, 1), more = TRUE)
print(update(plots$effects.logi.S, main = "QR decomposition", layout = c(2, 1), aspect = "fill"), split = c(2, 1, 2, 1), more = FALSE)
```
...
Each panel in the plot shows the theoretical zero line $\beta_{jg}=0$ under each of the two models (horizontal and vertical dashed lines).  The plotting symbols are color coded according to gene rank (rainbow, red to violet); the last "rank" #`r length(levels(Betas[[1]]$Gene))` (violet) corresponds to the weighted average `WA` of read count ratio over genes.  The plotting symbols also display the rank with numbers; these are `r paste0("(", seq_along(levels(Betas[[1]]$Gene)), ") ", levels(Betas[[1]]$Gene), collapse = ";  ")`.

```{r echo=FALSE}
mtype.compare.plot <- function(mtypeA = "logi.S", mtypeB = "wnlm.Q", dt = do.call(cbind, Betas), do.key = TRUE, ...) {
    mycol <- rainbow(n = length(levels(dt$wnlm.Q.Gene)), start = 0, end = 5 / 6)
    auto.key <-
        if(do.key)
            list(columns = 5, col = "black", points = FALSE, cex = 0.7,
                 text = paste0("(", seq_along(levels(Betas[[1]]$Gene)), ") ", levels(Betas[[1]]$Gene)))
        else
            FALSE
    ...
    xyplot(fm, data = dt,
           ...
           panel = function(x, y, subscripts, gene, ...){
...
mtype.compare.plot(mtypeA = "logi.S", mtypeB = "logi2.S") 
```
