...
levels(genes.ifat$imprinting.status) <- c("known", "candidate, <1MB", "candidate")
fraction.1 <-
    data.frame(lapply(rev(row.names(genes.ifat)),
                      pad.fraction.imprint, imp.stat = genes.ifat, fr = frac$min.reads.7[rev(row.names(genes.ifat))]),
               check.names = FALSE)
names(fraction.1) <- rev(row.names(genes.ifat))
```

```{r compare-to-ifats-fig, echo=FALSE, fig.width=7, fig.asp=2}
xlab <- "fraction of indiv."
par.set <- list(superpose.polygon = list(col = palette.ifat(cols = "darkorange", detailed = FALSE)))
trellis.par.set(superpose.polygon = list(lty = 0))
b.charts <-
    lapply(min.r.names,
           function(n)
               barchart(t(as.matrix(frac[[n]][nrow(genes.ifat):1])), par.settings = par.set, main = n, xlab = xlab))
...
par.set <- list(superpose.polygon = list(col = palette.ifat(detailed = FALSE)))
plot(barchart( t(as.matrix(fraction.1)), par.settings = par.set, main = "min.reads.7 + Ifat's ranking", xlab = xlab), split = c(2, 2, 2, 2), more = FALSE)
```
...
Notice that the lowest scoring gene in the "candidate, < 1MB" category is *PWRN1* (green on [Fig 1][ifat fig 1]).  Its score is `r 1 - ECDF$min.reads.15$PWRN1(0.9)` and it ranks at `r match("PWRN1", names(ECDF$min.reads.15))` according to $R_{15}$ and at `genes.ifat.ranks["PWRN1", "R.ifat"]`.  We may define the classification threshold such that we only call genes monoallelic if their $R_{15}$-ranking is above *PWRN1*.
```{r}
