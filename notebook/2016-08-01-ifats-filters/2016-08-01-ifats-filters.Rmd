...
# ECDF
fr <- frac$min.reads.15[nrow(genes.ifat):1]
annot <- c("*", "*", " ")
par.set <- list(superpose.polygon = list(col = palette.ifat(detailed = FALSE, col2 = "black")))
...
fig.ms$ecdf <- update(fig.ms$ecdf, scales = list(y = list(labels = paste(names(fr), annot))),
                      auto.key = list(text = c("known imprinted", "nearby candidate", "distant candidate"),
                                      points = FALSE, col = c("blue", "darkgreen", "red"), columns = 1))
fig.ms$tests <- update(fig.ms$tests, scales = list(y = list(alternating = 2)))
#print(fig.ms$ecdf, position = c(0, 0, 0.55, 1), more = TRUE)
#print(fig.ms$tests, position = c(0.5, 0, 1, 1), more = FALSE)
print(fig.ms$ecdf)
```
