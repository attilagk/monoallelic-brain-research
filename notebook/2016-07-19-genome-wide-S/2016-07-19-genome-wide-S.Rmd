...
```{r complex-plot, echo=FALSE, fig.asp=1.5, out.height=1000}
...
plots$density <- my.densityplot(S.long, par.settings = list(superpose.line = superpose.line), xlab = NULL, ylab = "density")
plots$density <- update(plots$density, scales = list(y = list(at = pretty(density(S[[sel.g[1]]], na.rm = TRUE)$y, n = 3))), par.settings = list(par.ylab.text = list(cex = 1.2)))
plots$ecdf <- my.ecdfplot(S.long, eval.at = c(0.9), survival = TRUE, par.settings =
                          list(superpose.line = superpose.line, par.ylab.text = list(cex = 1.2),
                               plot.symbol = list(pch = 21, col = "black", cex = 2, fill = "lightgreen")))
plots$level <- my.levelplot(ED.long, pct.top.g = 2, n.all.g = length(levels(ED.long$gene)), top.on.top = TRUE, xlab = "read count ratio, S", ylab = "gene rank")
...
plots <- lapply(plots, update, par.settings = list(par.xlab.text = list(cex = 1.2), par.ylab.text = list(cex = 1.2)))
plot.all(plots)
```

```{r complex-plot-b, echo=FALSE, fig.asp=1.0}
plot.all.b(plots)
```
