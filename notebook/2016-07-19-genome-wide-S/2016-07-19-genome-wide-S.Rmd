...
plots$rank <- rankplot(cum.fraction, plot.andys.test = FALSE, pct.top.g = 2, sel.g = sel.g,
...
```{r all-genes-on-one-scale, echo=FALSE, fig.width=7, fig.aspect=0.5}
levelplot(ECDF ~ s * gene,  data = ED.long, legend = NULL, colorkey = FALSE, scales = list(draw = FALSE), ylim = c(length(ok.genes), 1), par.settings = list(axis.line = list(lty = 0)))
```
...
genes.fig1 <- read.csv("ifat-fig1-genelist.csv", row.names = "gene")
genes.fig1$imprinting.status <- as.factor(genes.fig1$imprinting.status)
levels(genes.fig1$imprinting.status) <- c("known", "candidate, <1MB", "candidate")
genes.fig1 <- genes.fig1[row.names(genes.fig1) %in% sorted.genes, , drop = FALSE]

fraction.1 <-
    data.frame(lapply(rev(row.names(genes.fig1)),
                      pad.fraction.imprint, imp.stat = genes.fig1, fr = fraction[rev(row.names(genes.fig1))]),
               check.names = FALSE)
names(fraction.1) <- rev(row.names(genes.fig1))

plot(barchart( t(as.matrix(fraction.1)), par.settings = list(superpose.polygon = list(col = palette.ifat(detailed = FALSE))), main = "Ifat's style", xlab = xlab), split = c(1, 1, 2, 1), more = TRUE)
plot(barchart( t(as.matrix(fraction.1)), par.settings = list(superpose.polygon = list(col = palette.ifat(detailed = TRUE))), main = "Detailed", xlab = xlab), split = c(2, 1, 2, 1), more = FALSE)
```
