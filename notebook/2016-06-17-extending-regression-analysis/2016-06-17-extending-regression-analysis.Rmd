...
plot.betas(M$logi.S[f.ids$logi.S], coef.name = "Age", conf.lev = 0.99, main = "logi.S")
```
...
```{r beta-age-wnlm.R, dev=c('png','pdf'), echo=FALSE, warning=FALSE, message=FALSE}
plot.betas(M$wnlm.R[f.ids$wnlm.R], coef.name = "Age", conf.lev = 0.99, main = "wnlm.R")
```
...
par(mar = c(5,5,4,2))
plot(sapply(c("wnlm.R", "logi.S"),
            function(m) (sapply(rownames(f.ids)[f.ids$wnlm.R & f.ids$logi.S],
