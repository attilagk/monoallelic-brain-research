...
data4fit <- function(S.f=files$S, N.f=files$N, X.f=files$X,
    ...
    stat.Y <- function(Y, gns=names(Y), stat=mean, col.name="Y_stat", ...) {
    ...
    R.16 <- stat.Y(R, gns=names(R), stat=mean, col.name="R_g16", na.rm=TRUE)
    ...
    H <- as.data.frame(lapply(names(S), function(g) S[g] * N[g]))
    H.16 <- stat.Y(H, gns=names(H), stat=sum, col.name="H_g16", na.rm=TRUE)
    ...
    N.16 <- stat.Y(N, gns=names(N), stat=sum, col.name="N_g16", na.rm=TRUE)
...
m.nof <- list()
for(y in responses) {
    m.nof[[y]]$nlm <- f$nlm(y, d.nof)
    m.nof[[y]]$nlm2 <- f$nlm2(y, d.nof)
    m.nof[[y]]$logi <- f$logi(y, d.nof)
    m.nof[[y]]$logi2 <- f$logi2(y, d.nof)
}
rm(list = c("y"))
...
for(n in c(8, 13, 16)) {
    ...
    m.ag[[gn]]$logi <-
        glm(formula = mk.form(paste0("S_", gn)), family = binomial, data = d.ag[[gn]], weights = d.ag[[gn]]$N_g8)
    m.ag[[gn]]$logi2 <-
        glm(formula = mk.form(paste0("S_", gn, " * 2 - 1")), family = binomial, data = d.ag[[gn]], weights = d.ag[[gn]]$N_g8)
}
rm(list = c("n", "gn"))
