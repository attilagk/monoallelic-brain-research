...
```{r ll-surf-coefs, fig.asp=0.90, out.asp=0.90}
...
```{r ll-surf-genes, fig.asp=0.55, out.asp=0.55, echo=FALSE}
sp <- ll.surfaceplot(fm = formula(rel.log.L ~ beta.A * beta.B | gene), df = dat$genes,
               layout = c(3, 1),
               par.settings = list(regions = list(col = rev(trellis.par.get("regions")$col))),
