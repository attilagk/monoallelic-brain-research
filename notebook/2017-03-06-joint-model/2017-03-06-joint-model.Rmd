...
```{r cache=TRUE}
M5 <- update(M3, . ~ . - (scale(Age) + scale(RIN) + scale(Ancestry.1) | Gene) + (Dx + Gender + scale(Age) + scale(RIN) + scale(Ancestry.1) | Gene))
```
...
```{r}
t(as.matrix(sapply(av.5, function(a) summarize.anova(list(a)))))
```
