...
Based on the above findings the linear predictor for models $M3$ and $M4$ is defined as below, and $M3$ and $M4$ are fitted:
...
#av.3$Gender.Gene.slope <- # factor playing the role of slope doesn't make much sense
#    anova(M3, update(M1b, . ~ . + (Gender + scale(Age) + scale(RIN) + scale(Ancestry.1) | Gene)))
# Dx
av.3$Dx.fix <- anova(M3, update(M3, . ~ . + Dx))
av.3$Dx.ran <- anova(M3, m <- update(M3, . ~ . + (1 | Dx)))
av.3$Dx.Gene <- anova(m, update(m, . ~ . + (1 | Dx:Gene))
#av.3$Dx.Gene.slope <- # factor playing the role of slope doesn't make much sense
#    anova(M3, update(M1b, . ~ . + (Dx + scale(Age) + scale(RIN) + scale(Ancestry.1) | Gene)))
```
