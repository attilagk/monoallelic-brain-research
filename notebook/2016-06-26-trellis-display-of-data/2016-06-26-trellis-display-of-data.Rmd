...
P$s.age.n$lattice <-
    levelplot(log10(N) ~ Age + S | Gene, data = Y.long, alpha = 1.0, col = "white",
              ...
              par.settings = list(regions = list(col = colorRampPalette(c("blue", "red")))))
#
...
```{r reg-coef-logi.S, dev=c("png", "pdf"), fig.width=10, fig.asp=1, cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE}
my.segplot <- function(data = Betas$logi.S) {
    segplot(Gene ~ Lower.CL + Upper.CL | Coefficient, data = data,
            scales = list(x = list(relation = "free")),
            as.table = TRUE, center = data$Estimate, draw.bands = FALSE)
}
my.segplot(data = Betas$logi.S)
```

```{r reg-coef-wnlm.R, dev=c("png", "pdf"), fig.width=10, fig.asp=1, cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE}
my.segplot(data = Betas$wnlm.R)
```

```{r anova-logi.S, dev=c("png", "pdf"), fig.width=10, fig.asp=1, cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE}
dotplot(Gene ~ Deviance | Predictor, data = A.long$logi.S)
```

```{r anova-wnlm.R, dev=c("png", "pdf"), fig.width=10, fig.asp=1, cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE}
dotplot(Gene ~ Deviance | Predictor, data = A.long$wnlm.R)
```

```{r effects-logi.S, dev=c("png", "pdf"), fig.width=10, fig.asp=1, cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE}
dotplot(Gene ~ Effect | Coefficient, data = Ef.long$logi.S)
```

```{r effects-wnlm.R, dev=c("png", "pdf"), fig.width=10, fig.asp=1, cache=TRUE, warning=FALSE, echo=FALSE, message=FALSE}
dotplot(Gene ~ Effect | Coefficient, data = Ef.long$wnlm.R)
```
