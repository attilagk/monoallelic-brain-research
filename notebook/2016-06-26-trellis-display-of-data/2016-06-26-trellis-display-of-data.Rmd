...
dp <- densityplot(~ Q | Gene, groups = Gender, data = Q.long, plot.points = FALSE)[1:30]
update(my.update(dp, auto.key = list(title = "gender", text = c("Male", "Female"), columns = 2, points = TRUE, lines = FALSE)),
       par.settings = list(superpose.line = list(fill = trellis.par.get("superpose.symbol")$fill[c(2, 1)],
                                                   col = trellis.par.get("superpose.symbol")$col[c(2, 1)])),
...
dp <- bwplot(Gender ~ Q | Gene, data = Q.long)[1:30]
...
dp <- densityplot(~ S | Gene, groups = Dx, data = S.long, plot.points = FALSE, subset = S.long$Dx %in% c("Control", "SCZ"))[1:30]
...
dp <- bwplot(Dx ~ S | Gene, data = S.long)[1:30]
my.update(dp)
```
...
dp <- densityplot(~ Q | Gene, groups = Dx, data = Q.long, plot.points = FALSE, subset = Q.long$Dx %in% c("Control", "SCZ"))[1:30]
...
dp <- bwplot(Dx ~ Q | Gene, data = Q.long)[1:30]
update(my.update(dp), xlab = "Q")
```
...
densityplot(~ Q | RIN.sh, data = Q.long, groups = Dx, subset = Gene == "MEST",
...
bwplot(Dx ~ Q | RIN.sh, data = Q.long, subset = Gene == "MEST", scales = list(x = list(relation = "free")),
            auto.key=TRUE)
```
