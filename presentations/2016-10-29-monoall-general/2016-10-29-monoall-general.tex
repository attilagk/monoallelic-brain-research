...
\includeonlyframes{bsm,title,toc,imprint-mouse-devel,igf2-imprint-evol,sister-disorders,previous-age-studies,our-study,cmc,toc-current,filtering-calling,fitting-models,ll-surface,all-betas,orthogonality,identifiability,anova,betas-cluster,signif-genes,cf-overall-expression,summary,psych-encode,improving-model}
...
\begin{frame}[label=igf2-imprint-evol]{Imprinting and placentation}{}
\begin{columns}[t]
...
\item \(\mathcal{H}_0:\) no effect \only<2->{ \(\Leftrightarrow \beta_g = 0 \)}
\end{itemize}
\vfill
\only<2->{
\begin{equation*}
...
\includegraphics[scale=\ownfigscale]{figures/2016-08-21-likelihood-surface/explain-rll-levelplot-1.png}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.5\textwidth}
\begin{center}
\only<1->{agreement between models}

\includegraphics<1->[width=\columnwidth]{figures/2016-06-22-extending-anova/logi-S-filtered-wnlm-Q-compare-1.pdf}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=p-val]{p-values}
\includegraphics[scale=\ownfigscale]{figures/2016-10-03-permutation-test/p-values-1.pdf}
\end{frame}

\begin{frame}[label=signif-genes]{Significantly affected genes}
...
\begin{frame}[label=psych-encode]
\begin{center}
\includegraphics[height=0.6\textheight]{figures/from-others/psychENCODE.png}
\end{center}
\end{frame}
